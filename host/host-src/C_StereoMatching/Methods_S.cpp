...
)
{
    ...
    for (c = 0; c < cn * 2; c++, prow1 += width, prow2 += width)
    {
        ...
        for (x = width - maxX2; x < width - minX2; x++)
        {
            ...
            buffer1[x] = (PixType)v0;
            buffer1[x + width2] = (PixType)v1;
        }
        ...
        for (x = minX1; x < maxX1; x++)
        {
            ...
            for (; d < maxD; d++)
            {
                ...
                int v0 = buffer1[width - x - 1 + d];
                int v1 = buffer1[width - x - 1 + d + width2];
...
public:
        ...
        clipTab(NULL)
    {
        ...
        //allocate-----------------------------------
                static short Cbuf_new[109920];
                Cbuf = Cbuf_new;

                static short Sbuf_new[109920];
                Sbuf = Sbuf_new;

                static short hsumBuf_new[1428960];
                hsumBuf = hsumBuf_new;

                static short pixDiff_new[119920];
                pixDiff = pixDiff_new;

                static short disp2cost_new[1241];
                disp2cost = disp2cost_new;

                static short disp2ptr_new[1241];
                disp2ptr = disp2ptr_new;

                static uchar tempBuf_new[7446];
                tempBuf = tempBuf_new;

                static short Lr_new0_new[477984];
                Lr_new0 = Lr_new0_new;

                static short minLr_new0_new[4596];
                minLr_new0 = minLr_new0_new;

                static short Lr_new1_new[477984];
                Lr_new1 = Lr_new1_new;

                static short minLr_new1_new[4596];
...
#pragma HLS INTERFACE mode=m_axi depth=466616 port=image_left_test_data offset=slave
#pragma HLS INTERFACE mode=m_axi depth=466616 port=image_right_test_data offset=slave
#pragma HLS INTERFACE mode=m_axi depth=36 port=k_left_test_data offset=slave
#pragma HLS INTERFACE mode=m_axi depth=16 port=t_left_test_data offset=slave
#pragma HLS INTERFACE mode=m_axi depth=16 port=t_right_test_data offset=slave
#pragma HLS INTERFACE mode=m_axi depth=1866464 port=depth_map offset=slave
