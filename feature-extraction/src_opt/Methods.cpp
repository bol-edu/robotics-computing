  ...
  //          return;
   ...
   // cout << "i: " << keypoints[1].pt.x << endl;
        ...
        for (int i = 0; i < kpSize; i++)
            {
#pragma HLS PIPELINE
                if (mask_num[(int)(keypoints[i].pt.x + 0.5f)] == 0)//(mask.at<uchar>((int)( 0 /*keypoints[i].pt.y + 0.5f */ ), (int)(keypoints[i].pt.x + 0.5f)) == 0)
                {
                    rm[total] = i;
                    total++;
...
//#pragma HLS UNROLL factor=8
    for (level = 0; level < nLevels; ++level)
    {
        ...
        Size sz_mask(linfo.width, linfo.height/4);
        Size wholeSize(sz.width + border * 2, sz.height + border * 2);
...
//#pragma HLS ARRAY_PARTITION variable=ch_image complete dim=1
    ...
    mask.rows = 376/4;
    mask.cols = 1241;
    static unsigned char ch_mask[1241 * 376/4];
    mask.data = ch_mask;
