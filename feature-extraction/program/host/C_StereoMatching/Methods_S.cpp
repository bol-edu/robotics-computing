...
static void calcPixelCostBT__(const Mat_S& img1, const Mat_S& img2, int y,
...
public:
    ...
    void allocate(T*& ptr, size_t count, ushort alignment = sizeof(T))
    {
        ...
        allocate_S((void**)(&ptr), static_cast<ushort>(sizeof(T)), count, alignment);

    }

    template <typename T>
    void zeroFill_S(T*& ptr)
    {
        //CV_Assert(ptr);
        zeroFill_S_((void**)&ptr);
    }
...
private:
    BufferArea_S(const BufferArea_S&); // = delete
    BufferArea_S& operator=(const BufferArea_S&); // = delete
    void allocate_S(void** ptr, ushort type_size, size_t count, ushort alignment);
    void zeroFill_S_(void** ptr);
...
void* fastMalloc_S(size_t bufSize);
...
public:
    ...
    void real_allocate()
    {
        ...
        cout << udata << endl;
        raw_mem = fastMalloc_S(type_size * allocated_count);
...
#ifndef OPENCV_ENABLE_MEMORY_SANITIZER
BufferArea_S::BufferArea_S(bool safe_) :
    oneBuf(0),
    totalSize(0),
...
BufferArea_S::~BufferArea_S()
{
    release_S();
}

void BufferArea_S::allocate_S(void** ptr, ushort type_size, size_t count, ushort alignment)
{
...
void BufferArea_S::zeroFill_S_(void** ptr)
{
...
inline
Range_S::Range_S(int _start, int _end)
    : start(_start), end(_end) {}

inline
int Range_S::size() const
{
    return end - start;
}
...
Range_S Range_S::all()
{
    return Range_S(INT_MIN, INT_MAX);
}

static inline
bool operator == (const Range_S& r1, const Range_S& r2)
{
...
public:
    ...
    inline void clearLr(const Range_S& range = Range_S::all()) const
    {
        for (uchar i = 0; i < 2; ++i)
        {
            if (range == Range_S::all())
            {
    ...
    inline void clearSBuf(int row, const Range_S& range = Range_S::all()) const
    {
        if (range == Range_S::all())
...
static void computeDisparitySGBM__(const Mat_S& img1, const Mat_S& img2,
    Mat_S& disp1)
{
    int minDisparity = 0;
...
void compute(Mat_S& left, Mat_S& right, Mat_S& disp) //CV_OVERRIDE
{
    int minDisparity = 0;
    int numDisparities = 96;
    int SADWindowSize = 11;
...
Mat_S computeLeftDisparityMap(Mat_S img_left, Mat_S img_right, int matcher_name, bool rgb)
{
...
Mat_S calc_depth_map(Mat_S disp_left, Mat_S k_left, Mat_S t_left, Mat_S t_right, bool rectified)
{
    //Mat_S depth_map = Mat_S::ones(disp_left.rows, disp_left.cols, CV_32F);
    Mat_S depth_map;
    depth_map.create_ones();

    //Mat_S depth_map = Mat_S(disp_left.rows, disp_left.cols, CV_32F, Scalar(1, 0, 0));
    //Mat_S depth_map = Mat_S(disp_left.rows, disp_left.cols, CV_32F, 1);
...
{
	Mat_S img_left, img_right, k_left, t_left, t_right, depth;
	Mat_S disp;
    ...
    //Mat_S k_left, r_left, t_left;
    //Mat_S k_right, r_right, t_right;
