...
void sepFilter2D__(Mat src, Mat dst, int ddepth,
    ...
    double delta, int borderType , int wx, int wy)
{
 ...
 //   Mat contKernelX = kernelX;
  //  Mat contKernelY = kernelY;
    ...
    Mat kernelX1(Size(kernelX.cols + kernelX.rows - 1, 1), 5/*kernelX.type()*/, kernelX.data);
        Mat kernelY1(Size(kernelY.cols + kernelY.rows - 1, 1), 5/*kernelX.type()*/, kernelX.data);

        int _srcType = 0;//CV_MAT_TYPE(src.type());
            int _dstType = 0;//CV_MAT_TYPE(dst.type());
		  ...
		  /*  FilterEngine__proceed(this_,
		    ...
		    static float brows5[5120 / 4], brows6[5120 / 4], brows7[5120 / 4], brows8[5120 / 4], brows9[5120 / 4];
		    static float brows0[5120 / 4], brows1[5120 / 4], brows2[5120 / 4], brows3[5120 / 4], brows4[5120 / 4];
/*#pragma HLS array_partition variable=brows0 type=cyclic factor=10
#pragma HLS array_partition variable=brows1 type=cyclic factor=10
#pragma HLS array_partition variable=brows2 type=cyclic factor=10
#pragma HLS array_partition variable=brows3 type=cyclic factor=10
#pragma HLS array_partition variable=brows4 type=cyclic factor=10
#pragma HLS array_partition variable=brows5 type=cyclic factor=10
#pragma HLS array_partition variable=brows6 type=cyclic factor=10
#pragma HLS array_partition variable=brows7 type=cyclic factor=10
#pragma HLS array_partition variable=brows8 type=cyclic factor=10
#pragma HLS array_partition variable=brows9 type=cyclic factor=10*/
...
//#pragma HLS array_partition variable=this_.ringBuf type=cyclic factor=10
		    for (;; dstF += dststep * i, dy += i)
		    {
#pragma HLS loop_tripcount max=90
...
#pragma HLS loop_tripcount max=4
		            ...
		            //brow[1] = 0;
		            int ptr_idx = (bi * this_.bufStep / 4);
					uchar row[1247];// /*isSep ?*/ &this_.srcRow[0];//: brow;
//#pragma HLS array_partition variable=row type=cyclic factor=7
		            ...
		            for (int j = 0; j < (width1 - _dx2 - _dx1) * esz; j++)
					{
#pragma HLS loop_tripcount max=1241
#pragma HLS PIPELINE
						row[j + _dx1 * esz] = srcF[j];
					}
...
#pragma HLS PIPELINE
		        ...
		        int ptr_idx0 = (bi_[0] * this_.bufStep / 4);
				int ptr_idx1 = (bi_[1] * this_.bufStep / 4);
				int ptr_idx2 = (bi_[2] * this_.bufStep / 4);
				int ptr_idx3 = (bi_[3] * this_.bufStep / 4);
				int ptr_idx4 = (bi_[4] * this_.bufStep / 4);
				int ptr_idx5 = (bi_[5] * this_.bufStep / 4);
				int ptr_idx6 = (bi_[6] * this_.bufStep / 4);
				int ptr_idx7 = (bi_[7] * this_.bufStep / 4);
				int ptr_idx8 = (bi_[8] * this_.bufStep / 4);
				int ptr_idx9 = (bi_[9] * this_.bufStep / 4);


				for (int idx = 0; idx <  this_.bufStep/4; idx++)
				{
#pragma HLS PIPELINE
					brows0[idx] = this_.ringBuf[ptr_idx0 + idx];
					brows1[idx] = this_.ringBuf[ptr_idx1 + idx];
					brows2[idx] = this_.ringBuf[ptr_idx2 + idx];
					brows3[idx] = this_.ringBuf[ptr_idx3 + idx];
					brows4[idx] = this_.ringBuf[ptr_idx4 + idx];
					brows5[idx] = this_.ringBuf[ptr_idx5 + idx];
					brows6[idx] = this_.ringBuf[ptr_idx6 + idx];
					brows7[idx] = this_.ringBuf[ptr_idx7 + idx];
					brows8[idx] = this_.ringBuf[ptr_idx8 + idx];
					brows9[idx] = this_.ringBuf[ptr_idx9 + idx];
				}
		        /*uchar* ptr = &this_.ringBuf[0];
				...
				brows5 = ptr + bi_[5] * this_.bufStep;

				brows6 = ptr + bi_[6] * this_.bufStep;

				brows7 = ptr + bi_[7] * this_.bufStep;

				brows8 = ptr + bi_[8] * this_.bufStep;

				brows9 = ptr + bi_[9] * this_.bufStep;*/
    ...
    int borderType)
{
   ...
   // _dst.create(size, type);
    ...
    int sdepth = 0/*CV_MAT_DEPTH(type)*/, cn = 1;//CV_MAT_CN(type);
...
void detect_fast(Mat& _image, KeyPoint* keypoints,  int& kpSize, int* mask_num)
{
  ...
  //          return;
   ...
   // cout << "i: " << keypoints[1].pt.x << endl;
        ...
        for (int i = 0; i < kpSize; i++)
            {
#pragma HLS PIPELINE
                if (mask_num[(int)(keypoints[i].pt.x + 0.5f)] < 255)//(mask.at<uchar>((int)( 0 /*keypoints[i].pt.y + 0.5f */ ), (int)(keypoints[i].pt.x + 0.5f)) == 0)
                {
                    rm[total] = i;
                    total++;
...
static void computeKeyPoints(const Mat& imagePyramid,
    ...
    int fastThreshold, int& outkpSize, int* mask_array)
{
    ...
    for (level = 0; level < nlevels; level++)
    {
       ...
       ///////    mask      ///////
         ...
         /*     mask.flags = maskPyramid.flags;
             ...
             // cout<<"i: "<<i<<endl;
              int* mask_num = mask_array + level * 1241;
        ...
        // Detect FAST features, 20 is a good threshold
        {
            ...
            detect_fast(img, keypoints, kpSize, mask_num); //Fast角點檢測
        ...
        retainBestHarris(keypoints, scoreType == 0 ? 2 * featuresNum : featuresNum, kpSize);
	...
	uchar* des_data, bool useProvidedKeypoints, int& kpSize, uchar* imgPyramid_data)
{
   ...
   // mask.data[0] = 0;
    //image.data = imgPyramid_data + (32*1312);
...
//#pragma HLS UNROLL factor=8
    for (level = 0; level < nLevels; ++level)
    {
        ...
        Size sz_mask(linfo.width, linfo.height/6);
        Size wholeSize(sz.width + border * 2, sz.height + border * 2);
    ...
    /*	   extMask.flags = maskPyramid.flags;
    	   ...
    	   Rect MaskLinfo = Rect(border, border, sz_mask.width, sz_mask.height);
    	   ...
    	   currMask.data = mask.data + ( level /*wholeLinfo.y + 32*/) * mask.step[0];
       ...
       // Compute the resized image
        if (level != firstLevel)  //得到金字塔每層的圖像
        {
        	...
        	resize__(prevImg, currImg, sz, 0, 0, INTER_LINEAR_EXACT);
             	 	 	 ...
             	 	 	 //if (!mask.empty())
                resize__(prevMask, currMask, sz_mask, 0, 0, INTER_LINEAR_EXACT);
        ...
        for (int c = 0; c < sz.width; c++)
			mask_num[c] = prevMask.at<uchar>(0, c);
   ...
   /* for (int j = 0; j < 1859 * 1312; j++)
    ...
    if (do_keypoints) //提取角點  
    {
        ...
        computeKeyPoints(imagePyramid, //maskPyramid,  // need level8
            ...
            nfeatures, scaleFactor, edgeThreshold, patchSize, scoreType, fastThreshold, kpSize, mask_array);
    	//kpSize = 500;
    	/*    if (imgPyramid_data[1859000] == 0 && imgPyramid_data[10000] == 1)
    	    	{
    	    		for (int v = 0; v < 500; v++)
    	    	       {
    	#pragma HLS PIPELINE
    	    			keypoints[v].pt.x = v;
    	    			keypoints[v].pt.y = v/2;
    	    	       }
    	    ...
    	    else
    	    {
    	    		for (int v = 0; v < 500; v++)
    	    	    	       {
    	#pragma HLS PIPELINE
    	    			keypoints[v].pt.x = 1+v;
    	    			keypoints[v].pt.y = 1+v/2;
    	    	    	       }
    	    }*/
  ...
  /*  for (level = 0; level < nLevels; ++level)
    {
        ...
        // cout << "wholeLinfo.x: " << wholeLinfo.x << " extImginfo: " << extImginfo.x << endl;
	...
	if (do_descriptors)
	{
	   ...
	   // if (wta_k == 2)
		...
		for (level = 0; level < nLevels; level++)
		{
			...
			//xf::cv::Mat<XF_8UC1, 376, 1241, XF_NPPC1, 1> in_mat(376, 1241);
...
{
#pragma HLS INTERFACE mode=m_axi depth=466616 bundle=BUS_A port=image_data offset=slave
#pragma HLS INTERFACE mode=m_axi depth=466616 bundle=BUS_B port=mask_data offset=slave
#pragma HLS INTERFACE mode=m_axi depth=1000 bundle=BUS_A port=kp_xy offset=slave
#pragma HLS INTERFACE mode=m_axi depth=16000 bundle=BUS_B port=des_data offset=slave
...
#pragma HLS allocation function instances=runByImageBorder limit=1
#pragma HLS allocation function instances=retainBestHarris limit=1
...
//#pragma HLS ARRAY_PARTITION variable=ch_image complete dim=1
    ...
    mask.rows = 376/6;
    mask.cols = 1241;
    static unsigned char ch_mask[1241 * 376/6];
    mask.data = ch_mask;
...
#pragma HLS PIPELINE
			image.at<uchar>(i,j) = image_data[i * 1241 + j];
			//mask.data[i] = mask_data[i];
		}
	}

    for(int i = 0; i < 376*1241/6; i++)
    	{
    		#pragma HLS PIPELINE
    		mask.data[i] = mask_data[i];
    	}

  /*  for(int i = 0; i < 1312*1859; i++)
        	{
        		#pragma HLS PIPELINE
    	int x = i%(376*1241);
    	imgP[i] = image.data[x];
   ...
   // uchar des_buf[500*32];
    //uchar* des = des_buf;
	detect(image, kp, mask, kpSize, imgP);
    //cout << "detect end" << endl;
	compute(imgP,  kp, mask.data, kpSize );
    //cout << "compute end" << endl;

   for(int o = 0; o < 500 * 32; o++)
   {
#pragma HLS PIPELINE
            des_data[o] = mask.data[o];
   }
