...
/*template<int num, typename T, size_t BinsOnStack = 0u>
static double getThreshVal_Otsu(const Mat& _src, const Size& size)
{   
    //const int N = numeric_limits<T>::max() + 1;
    const int N = num + 1;
    int i, j;
...
getThreshVal_Otsu_8u(const Mat& _src)
{
    ...
    return getThreshVal_Otsu<255,uchar, 256u>(_src, size);
}*/
...
getThreshVal_Otsu_16u(const Mat& _src)
{
    ...
    return getThreshVal_Otsu<65535, ushort>(_src, size);
}
...
/*class CV_EXPORTS ParallelLoopBody_
{
public:
...
/*CV_EXPORTS void parallel_for__(const Range& range, const ParallelLoopBody& body, double nstripes = -1.);
...
/*void parallel_for__(const cv::Range& range, const ParallelLoopBody_& body, double nstripes)
{
#ifdef OPENCV_TRACE
...
double threshold__(InputArray _src, OutputArray _dst, double thresh, double maxval, int type)
{
