...
int main()
{
	...
	***************************************************************************************/
	...
	bool stereo_matcher = BM;
	int subset = 6;

	cv::Mat mask = cv::Mat::zeros(handler.imheight, handler.imwidth, CV_8U);
	mask(Rect(96, 0, handler.imwidth - 96, handler.imheight)) = 255;

	vector<cv::Mat> trajectory_nolidar_bm =
		method.visual_odometry(handler, Sift, BF, 0.3, BM, subset, mask);

	FILE* fp;
	fopen_s(&fp, "../output1/trajectory_nolidar_test.txt", "w");
...
/*	Mat des0, des1;
	...
	cout << "We are in the tutorial!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << endl;
...
*/
	...
	***********************************************************************************/
/*	vector<vector<DMatch>> matches = method.match_features(des0, des1, BF, Orb, true, 2);
...
/*	depth = method.stereo_2_depth(first_img_left,
	...
	system("cls");
*/
	/*des0 = test1.extract_features(first_img_left, Orb, mask, &kp0);
	des1 = test1.extract_features(second_img_left, Orb, mask, &kp1);*/
/*	des0 = method.extract_features(first_img_left, Orb, mask, &kp0);
	des1 = method.extract_features(second_img_left, Orb, mask, &kp1);
