...
{
#pragma HLS INTERFACE m_axi port = des0 depth = 16000 offset = slave
#pragma HLS INTERFACE m_axi port = des1 depth = 16000 offset = slave
#pragma HLS INTERFACE m_axi port = match depth = 500 offset = slave
#pragma HLS INTERFACE m_axi port = match_num depth = 1 offset = slave
...
#pragma HLS INTERFACE s_axilite port = return
    ...
    for (int i = 0; i < MAX_KEYPOINT_NUM; i++)
    {
        ...
        {
            match[count].a = matches[i][0].queryIndex;
            match[count].b = matches[i][0].trainIndex;
            count++;
        }
    }
    ...
    for (int i = count; i < MAX_KEYPOINT_NUM; i++)
    {
    	match[i].a = 0;
    	match[i].b = 0;
    }
