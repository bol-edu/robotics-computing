...
namespace xf
{
	namespace solver
	{
		...
		struct svdTraits
		{
			...
			static const int NUM_SWEEPS = 30; // Literature typically suggestions 6 to 10 iterations to successfully converge
...
#pragma HLS PIPELINE II = SVDTraits::OFF_DIAG_II
		...
		col_load:
			...
			row_load:
				for (int row = 0; row < size + odd_dim; row++)
				{
#pragma HLS PIPELINE
					if (odd_dim)
					{
		...
		sweep_loop:
			...
			step_loop:
				for (int step = 0; step < num_steps; step++)
				{
					...
					int input_bank = ((sweepnum * num_steps) + step) % BANKS;	   /* !!! */
					int output_bank = ((sweepnum * num_steps) + step + 1) % BANKS; /* !!! */
...
#pragma HLS PIPELINE
					...
					int size)
		{
			...
			case 0:
				...
				break;
			case 1:
				svdPairs<RowsA, ColsA, SVDTraits, InputType, OutputType>(A, S, U, V, size);
				break;
			default:
				svdBasic<RowsA, ColsA, SVDTraits, InputType, OutputType>(A, S, U, V);
				break;
		...
		void svd(T* A_input, T* S_output, T* U_output, T* V_output, int size)
		{
			const int MAX_SIZE = 12;
			T A[MAX_SIZE][MAX_SIZE];
			T S[MAX_SIZE][MAX_SIZE];
			T U[MAX_SIZE][MAX_SIZE];
			T V[MAX_SIZE][MAX_SIZE];

			for (int i = 0; i < size; i++)
				for (int j = 0; j < size; j++)
					A[i][j] = A_input[i*size+j];

			svdPairs<MAX_SIZE, MAX_SIZE, svdTraits<MAX_SIZE, MAX_SIZE, T, T>, T, T>(A, S, U, V, size);

			for (int i = 0; i < size; i++)
				S_output[size-i-1] = S[i][i];

			for (int i = 0; i < size; i++)
				for (int j = 0; j < size; j++)
					U_output[i*size+size-j-1] = U[i][j];

			for (int i = 0; i < size; i++)
				for (int j = 0; j < size; j++)
					V_output[i*size+size-j-1] = V[i][j];
		}
