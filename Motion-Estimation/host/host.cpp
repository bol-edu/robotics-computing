...
// ------------------------------------------------------------------
    ...
    // .......................................................
    GlobMem_BUF_rmat = clCreateBuffer(Context, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR, 9 * sizeof(FLOAT), rmat, &errCode);
    if (errCode != CL_SUCCESS)
    {
        cout << endl
    ...
    // .......................................................
    GlobMem_BUF_tvec = clCreateBuffer(Context, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR, 3 * sizeof(FLOAT), tvec, &errCode);
    if (errCode != CL_SUCCESS)
    {
        cout << endl
...
// ----------------------------------------
    ...
    errCode |= clSetKernelArg(EstimateMotion, 9, sizeof(cl_mem), &GlobMem_BUF_rmat);
    errCode |= clSetKernelArg(EstimateMotion, 10, sizeof(cl_mem), &GlobMem_BUF_tvec);
    ...
    vector<cv::Mat> trajectory(num_frames);
...
// ============================================================================
        ...
        cv::Mat Rmat = cv::Mat(3, 3, CV_32F, rmat);
        cv::Mat Tvec = cv::Mat(3, 1, CV_32F, tvec);
        ...
        T_tot(rect).copyTo(trajectory[ii + 1]);
    }
    // ------------------------------------------------------
    ...
    FILE *fp;
    fp = fopen("../../../output/trajectory.txt", "w");

    for (int i = 0; i < trajectory.size(); i++)
    {
        fprintf(fp, "%f\t%f\t%f\n",
                trajectory.at(i).at<float>(0, 3),
                trajectory.at(i).at<float>(1, 3),
                trajectory.at(i).at<float>(2, 3));
    }
    fclose(fp);
               ...
               FLOAT &fx, FLOAT &fy, FLOAT &cx, FLOAT &cy, FLOAT *depth, int idx)
{
    ...
    sprintf(kp0_name, "../../../testdata/kp0_%d.txt", idx);
    sprintf(kp1_name, "../../../testdata/kp1_%d.txt", idx);
    sprintf(depth_name, "../../../testdata/depth_%d.txt", idx);
