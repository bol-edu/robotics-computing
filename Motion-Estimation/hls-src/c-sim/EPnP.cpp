...
void EPnP::compute(Matrix &rmat, Matrix &tvec)
{
    ...
    for (int32_t i = 0; i < number_of_correspondences; i++)
    ...
    {
        FLOAT k = sqrt(DC.val[i - 1][0] / number_of_correspondences);
        for (int32_t j = 0; j < 3; j++)
                            ...
                            Matrix &R, FLOAT t[3])
{
    ...
    for (int32_t i = 0; i < number_of_correspondences; i++)
    {
        FLOAT Xc = Matrix::dot(R.val[0], pws.val[i]) + t[0];
        FLOAT Yc = Matrix::dot(R.val[1], pws.val[i]) + t[1];
        FLOAT inv_Zc = 1.0 / (Matrix::dot(R.val[2], pws.val[i]) + t[2]);
