                ...
                FLOAT rmat[9], FLOAT tvec[3])
{
    ...
    for (int i = 0; i < point_num; i++)
    {
        if (best_mask[i])
        {
            opoint_inlier[j].x = opoint[i].x;
            opoint_inlier[j].y = opoint[i].y;
            opoint_inlier[j].z = opoint[i].z;

            ipoint_inlier[j].x = ipoint[i].x;
            ipoint_inlier[j].y = ipoint[i].y;
            j++;
        }
    }

    pnp_iterative(opoint_inlier, ipoint_inlier, j, fx, fy, cx, cy, rmat, tvec);
}
