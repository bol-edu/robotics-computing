...
{
    FILE* fpt_level;
    fopen_s(&fpt_level, "../output/disp5.txt", "w");
    ...
    for (int j = 0; j < extImg.rows; j++)
    {
        for (int k = 0; k < extImg.cols; k++)
        {
            ...
            //float temp = extImg.at<float>(j, k);
            //fprintf_s(fpt_level, " data: %f ", temp);
            fprintf_s(fpt_level, " %d ", extImg.data[k + (j * imcol_level)]);
            //cout << image_left.at<unsigned char>(j, k) << endl;
            fprintf_s(fpt_level, "\n");
        }
    }*/
    for (int i = 0; i < extImg.cols * extImg.rows * 4+5; i++)
    {
        fprintf_s(fpt_level, "%d: %d\n", i, extImg.data[i]);
    }

    fclose(fpt_level);
}
/*
class CV_EXPORTS Algorithm__my;
...
class CV_EXPORTS_W Algorithm__my
{
public:
    Algorithm__my();*/
    //virtual ~Algorithm__my();
...
/*
void Algorithm__my::write(const Ptr<FileStorage>& fs, const String& name) const
{
    //CV_TRACE_FUNCTION();
    if (name.empty())
    {
...
/*
class CV_EXPORTS_W StereoMatcher__ : public Algorithm__my
{
public:
    enum {
        DISP_SHIFT = 4,
    ...
    CV_WRAP virtual void compute(Mat& left, Mat& right,
        Mat& disparity) = 0;
...
inline size_t parseOption(const std::string& value)
{
    ...
    std::string valueStr = value.substr(0, pos);
    std::string suffixStr = value.substr(pos, value.length() - pos);
...
//----------------------------------------------------------------------------------------------//
/*
CV_EXPORTS Mat& noArray__();

CV_EXPORTS_W void filterSpeckles__(Mat& img, double newVal,
    int maxSpeckleSize, double maxDiff,
    Mat& buf = noArray__());

void filterSpeckles__(Mat& img, double _newval, int maxSpeckleSize,
    double _maxDiff, Mat& __buf)
{
...
/*
    ...
    int getMode() const CV_OVERRIDE { return params.mode; }disp.create
...

{
    ...
    else */if (matcher_name == SGBM)
    {
        ...
        /*
        cout << "before_type:" << disp_left.type() << endl;
        cout << "before_cols:" << disp_left.cols << endl;
        cout << "before_rows:" << disp_left.rows << endl;
        cout << "before_step0:" << disp_left.step.p[0] << endl;
        cout << "before_step1:" << disp_left.step.p[1] << endl;
        cout << "before_size:" << disp_left.size << endl;
        cout << "before_data0:" << static_cast<int>(disp_left.data[0]) << endl;
        cout << "before_data1:" << static_cast<int>(disp_left.data[1]) << endl;
        cout << "before_data2:" << static_cast<int>(disp_left.data[2]) << endl;
        cout << "before_data3:" << static_cast<int>(disp_left.data[3]) << endl;
        cout << "before_data4:" << static_cast<int>(disp_left.data[4]) << endl;
        cout << disp_left.at<short>(0, 0) << endl;
        cout << disp_left.at<short>(20, 300) << endl;
        cout << disp_left.at<short>(55, 700) << endl;
        cout << disp_left.at<short>(30, 40) << endl;*/
        //outmat(disp_left);
        ...
        outmat(disp_left);
        /*
        cout << "after_type:" << disp_left.type() << endl;
        cout << "after_cols:" << disp_left.cols << endl;
        cout << "after_rows:" << disp_left.rows << endl;
        cout << "after_step0:" << disp_left.step.p[0] << endl;
        cout << "after_step1:" << disp_left.step.p[1] << endl;
        cout << "after_size:" << disp_left.size << endl;
        cout << "after_data0:" << static_cast<int>(disp_left.data[0]) << endl;
        cout << "after_data1:" << static_cast<int>(disp_left.data[1]) << endl;
        cout << "after_data2:" << static_cast<int>(disp_left.data[2]) << endl;
        cout << "after_data3:" << static_cast<int>(disp_left.data[3]) << endl;
        cout << "after_data4:" << static_cast<int>(disp_left.data[4]) << endl;
        cout << disp_left.at<float>(0, 0) << endl;
        cout << disp_left.at<float>(20, 300) << endl;
        cout << disp_left.at<float>(55, 700) << endl;
        cout << disp_left.at<float>(30, 40) << endl;*/
...
***************************************************************************************/
{
    ...
    //Mat depth_map = Mat(disp_left.rows, disp_left.cols, CV_32F, Scalar(1, 0, 0));//Mat depth_map = Mat(disp_left.rows, disp_left.cols, CV_32F, 1);
...
Mat Methods::calc_depth_map_1(Mat disp_left, Mat k_left, Mat t_left, Mat t_right, bool rectified)
/***************************************************************************************
    Calculate depth map using a disparity map, intrinsic camera matrix, and translation
    vectors from camera extrinsic matrices(to calculate baseline).
    Note that default behavior is for rectified projection matrix for right camera.
...
***************************************************************************************/
{
    ...
    FILE* fpd2123;
    fopen_s(&fpd2123, "../output/test_depth_map.txt", "w");

    fprintf_s(fpd2123, "%d\t%d\t%lld\t%lld\t",
        depth_map.flags,
        depth_map.dims,
        depth_map.step.p[0],
        depth_map.step.p[1]
    );
    fprintf_s(fpd2123, "%d\t%d\n", depth_map.rows, depth_map.cols);


    for (int j = 0; j < depth_map.rows; j++)
    {
        for (int k = 0; k < depth_map.cols; k++)
        {
            fprintf_s(fpd2123, " %d ", j);
            fprintf_s(fpd2123, " %d ", k);
            fprintf_s(fpd2123, " %f ", depth_map.at<float>(j, k));
            fprintf_s(fpd2123, " %d ", depth_map.data[k + (j * depth_map.cols)]);
    ...
    //cout << disp_left.rows << " " << disp_left.cols << endl;
    ...
    for (int i = 0; i < disp_left.rows; i++)
    {
        for (int j = 0; j < disp_left.cols; j++)
        {
            ...
            /*if (i == 0 && j == 0)
            {
                ...
                FILE* fpd213;
                fopen_s(&fpd213, "../output/test_depth_at.txt", "w");
        ...
        //cout << "(" << i << ", " << "0" << "): " << static_cast<float>(depth_map.at<float>(i, 0)) << endl;
    }
    /*
    FILE* fpd2123;
    fopen_s(&fpd2123, "../output/test_depth_map1.txt", "w");
    ...
    for (int j = 0; j < depth_map.rows; j++)
    {
        for (int k = 0; k < depth_map.cols; k++)
        {
            ...
            fprintf_s(fpd2123, " %d ", k);
            fprintf_s(fpd2123, " %f ", depth_map.at<float>(j, k));
            //fprintf_s(fpd2123, " %d ", depth_map123.data[k + (j * depth_map123.cols)]);
            //cout << "type: " << depth.type() << endl;//5
            fprintf_s(fpd2123, "\n");
        }
    }
...
Mat Methods::stereo_2_depth_1(Mat img_left, Mat img_right, Mat P0, Mat P1, bool matcher, bool rgb, bool rectified)
/***************************************************************************************
    Takes stereo pair of images and returns a depth map for the left camera.If your
    projection matrices are not rectified, set rectified = false.
...
***************************************************************************************/
{
    ...
    /*
    FILE* fpm7;
    fopen_s(&fpm7, "../output/matdata_t_right_final.txt", "w");

    fprintf_s(fpm7, "%d\t%d\t%lld\t%lld\t",
        t_right.flags,
        t_right.dims,
        t_right.step.p[0],
        t_right.step.p[1]
    );
    fprintf_s(fpm7, "%d\t%d\n", t_right.rows, t_right.cols);

    //int imcolkk = t_right.cols;

    for (int j = 0; j < t_right.rows; j++)
    {
        for (int k = 0; k < t_right.cols; k++)
        {
            fprintf_s(fpm7, " %d ", j);
            fprintf_s(fpm7, " %d ", k);
            fprintf_s(fpm7, " %f ", t_right.at<float>(j, k));
            //fprintf_s(fpm7, " %d ", t_right.data[k + (j * imcolkk)]);
            //cout << "type: " << t_right.type() << endl;//5
            fprintf_s(fpm7, "\n");
        }
    }
    ...
    FILE* fpm8;
    fopen_s(&fpm8, "../output/matdata_k_left_final.txt", "w");

    fprintf_s(fpm8, "%d\t%d\t%lld\t%lld\t",
        k_left.flags,
        k_left.dims,
        k_left.step.p[0],
        k_left.step.p[1]
    );
    fprintf_s(fpm8, "%d\t%d\n", k_left.rows, k_left.cols);

    //int imcolkk = k_left.cols;

    for (int j = 0; j < k_left.rows; j++)
    {
        for (int k = 0; k < k_left.cols; k++)
        {
            fprintf_s(fpm8, " %d ", j);
            fprintf_s(fpm8, " %d ", k);
            fprintf_s(fpm8, " %f ", k_left.at<float>(j, k));
            //fprintf_s(fpm8, " %d ", k_left.data[k + (j * k_left.cols * 4)]);
            //cout << "type: " << k_left.type() << endl;//5
            fprintf_s(fpm8, "\n");
        }
    }
    ...
    FILE* fpm9;
    fopen_s(&fpm9, "../output/matdata_t_left_final.txt", "w");

    fprintf_s(fpm9, "%d\t%d\t%lld\t%lld\t",
        t_left.flags,
        t_left.dims,
        t_left.step.p[0],
        t_left.step.p[1]
    );
    fprintf_s(fpm9, "%d\t%d\n", t_left.rows, t_left.cols);

    for (int j = 0; j < t_left.rows; j++)
    {
        for (int k = 0; k < t_left.cols; k++)
        {
            fprintf_s(fpm9, " %d ", j);
            fprintf_s(fpm9, " %d ", k);
            fprintf_s(fpm9, " %f ", t_left.at<float>(j, k));
            //cout << "type: " << t_left.type() << endl;//5
            fprintf_s(fpm9, "\n");
        }
    }
    //----------------------------------------------------------------
    ...
    FILE* fpmright;
    fopen_s(&fpmright, "../output/matdata_t_right_final.txt", "w");

    fprintf_s(fpmright, "%d\t%d\t%lld\t%lld\t",
        t_right.flags,
        t_right.dims,
        t_right.step.p[0],
        t_right.step.p[1]
    );
    fprintf_s(fpmright, "%d\t%d\n", t_right.rows, t_right.cols);

    //int imcolkk = t_right.cols;

        for (int k = 0; k < t_right.cols* t_right.rows*4; k++)
        {
            fprintf_s(fpmright, " %d ", t_right.data[k]);
            fprintf_s(fpmright, "\n");
        }
    ...
    /*
    FILE* fpd1d;
    fopen_s(&fpd1d, "../output/test_disp123.txt", "w");

    fprintf_s(fpd1d, "%d\t%d\t%lld\t%lld\t",
        disp.flags,
        disp.dims,
        disp.step.p[0],
        disp.step.p[1]
    ...
    {
        for (int k = 0; k < disp.cols; k++)
        {
            fprintf_s(fpd1d, " %d ", j);
            fprintf_s(fpd1d, " %d ", k);
            fprintf_s(fpd1d, " %f ", disp.at<float>(j, k));
            fprintf_s(fpd1d, " %d ", disp.data[k + (j * k_left.cols)]);
            //cout << "type: " << disp_left_origin.type() << endl;//5
            fprintf_s(fpd1d, "\n");
        }
    }
    ...
    /*
    FILE* fpt1k;
    fopen_s(&fpt1k, "../output/t_left_test.txt", "w");
    fprintf_s(fpt1k, "flags: %d\tdims: %d\tstep: %lld\t",
        t_left.flags,
        t_left.dims,
        t_left.step.p[0]);
    fprintf_s(fpt1k, "%lld\n ", t_left.step.p[1]);
    ...
    //int imcol3k = k_left_test.cols;

    for (int k = 0; k < 8; k++)
    {
        fprintf_s(fpt1k, " %d ", t_left.data[k]);
    }

    fclose(fpt1k);

    FILE* fpt1k1;
    fopen_s(&fpt1k1, "../output/t_right_test.txt", "w");
    fprintf_s(fpt1k1, "flags: %d\tdims: %d\tstep: %lld\t",
        t_right.flags,
        t_right.dims,
        t_right.step.p[0]);
    fprintf_s(fpt1k1, "%lld\n ", t_right.step.p[1]);
    ...
    for (int k = 0; k < 8; k++)
    {
        fprintf_s(fpt1k1, " %d ", t_right.data[k]);
    }
    ...
    // Calculate depth map for left camera
    Mat depth = calc_depth_map_1(disp, k_left, t_left, t_right, true);
    ...
    ***************************************************************************************/
{
    ...
    for (int i = 0; i < num_frames - 1; i++)
    {
        ...
        {   /*
            FILE* fpm;
            fopen_s(&fpm, "../output/testmat.txt", "w");

            fprintf_s(fpm, "%d\t%d\t%d :\n",
                i,
                i,
                i);
            fprintf_s(fpm, "flags: %d\tdims: %d\tstep: %lld\t",
                image_left.flags,
                image_left.dims,
                image_left.step.p[0]);
            fprintf_s(fpm, "%lld\n", image_left.step.p[1]);

            int imcol = image_left.cols;
            for (int j = 0; j < image_left.rows; j++)
            {
                for (int k = 0; k < image_left.cols; k++)
                {
                    fprintf_s(fpm, " ( ");
                    fprintf_s(fpm, " %d, ", j);
                    fprintf_s(fpm, " %d ): ", k);
                    fprintf_s(fpm, " data: %d ", image_left.at<unsigned char>(j, k));
                    fprintf_s(fpm, " %d ", image_left.data[k + (j * imcol)]);
                    fprintf_s(fpm, "\n");
                }
            }
            ...
            FILE* fpm1;
            fopen_s(&fpm1, "../output/matdata_left.txt", "w");

            fprintf_s(fpm1, "%d\t%d\t%lld\t%lld\t",
                image_left.flags,
                image_left.dims,
                image_left.step.p[0],
                image_left.step.p[1]
            ...
            for (int j = 0; j < image_left.rows; j++)
            {
                for (int k = 0; k < image_left.cols; k++)
                {
                    fprintf_s(fpm1, " %d ", j);
                    fprintf_s(fpm1, " %d ", k);
                    fprintf_s(fpm1, " %d ", image_left.at<unsigned char>(j, k));
                    fprintf_s(fpm1, "\n");
                }
            }
            ...
            FILE* fpm2;
            fopen_s(&fpm2, "../output/matdata_right.txt", "w");

            fprintf_s(fpm2, "%d\t%d\t%lld\t%lld\t",
                image_right.flags,
                image_right.dims,
                image_right.step.p[0],
                image_right.step.p[1]
            );
            fprintf_s(fpm2, "%d\t%d\n", image_right.rows, image_right.cols);

            for (int j = 0; j < image_right.rows; j++)
            {
                for (int k = 0; k < image_right.cols; k++)
                {
                    fprintf_s(fpm2, " %d ", j);
                    fprintf_s(fpm2, " %d ", k);
                    fprintf_s(fpm2, " %d ", image_right.at<unsigned char>(j, k));
                    //cout << "type: " << image_right.type() << endl;//0
                    fprintf_s(fpm2, "\n");
                }
            }
            ...
            /*
            FILE* fpm3;
            fopen_s(&fpm3, "../output/matdata_k_left.txt", "w");

            fprintf_s(fpm3, "%d\t%d\t%lld\t%lld\t",
                k_left.flags,
                k_left.dims,
                k_left.step.p[0],
                k_left.step.p[1]
            ...
            for (int j = 0; j < k_left.rows; j++)
            {
                for (int k = 0; k < k_left.cols; k++)
                {
                    fprintf_s(fpm3, " %d ", j);
                    fprintf_s(fpm3, " %d ", k);
                    fprintf_s(fpm3, " %f ", k_left.at<float>(j, k));
                    //fprintf_s(fpm3, " %d ", k_left.data[k + (j * imcolkk)]);
                    //cout << "type: " << k_left.type() << endl;//5
                    fprintf_s(fpm3, "\n");
                }
            }
            ...
            FILE* fpm4;
            fopen_s(&fpm4, "../output/matdata_r_left.txt", "w");

            fprintf_s(fpm4, "%d\t%d\t%lld\t%lld\t",
                r_left.flags,
                r_left.dims,
                r_left.step.p[0],
                r_left.step.p[1]
            );
            fprintf_s(fpm4, "%d\t%d\n", r_left.rows, r_left.cols);

            for (int j = 0; j < r_left.rows; j++)
            {
                for (int k = 0; k < r_left.cols; k++)
                {
                    fprintf_s(fpm4, " %d ", j);
                    fprintf_s(fpm4, " %d ", k);
                    fprintf_s(fpm4, " %f ", r_left.at<float>(j, k));
                    //cout << "type: " << r_left.type() << endl;//5
                    fprintf_s(fpm4, "\n");
                }
            }
            ...
            FILE* fpm5;
            fopen_s(&fpm5, "../output/matdata_t_left.txt", "w");

            fprintf_s(fpm5, "%d\t%d\t%lld\t%lld\t",
                t_left.flags,
                t_left.dims,
                t_left.step.p[0],
                t_left.step.p[1]
            ...
            for (int j = 0; j < t_left.rows; j++)
            {
                for (int k = 0; k < t_left.cols; k++)
                {
                    fprintf_s(fpm5, " %d ", j);
                    fprintf_s(fpm5, " %d ", k);
                    fprintf_s(fpm5, " %f ", t_left.at<float>(j, k));
                    //cout << "type: " << t_left.type() << endl;//5
                    fprintf_s(fpm5, "\n");
                }
            }
...
#pragma warning(disable:4996)
            FILE* fpmr;
            fpmr = fopen("../output/matdata_left.txt", "r");
            int vnum1, vnum2;
            int data;
            ...
            fscanf(fpmr, "%d %d %lld %lld %d %d", &flag, &dim, &p1, &p2, &row, &col);

            //image_test = image_left;
           
            image_left_test.flags = flag;
            image_left_test.dims = dim;
            image_left_test.step.p[0] = p1;
            image_left_test.step.p[1] = p2;
            image_left_test.rows = row;
            image_left_test.cols = col;
            static unsigned char ch2[1241 * 376];

            image_left_test.data = ch2;
            int imcol4 = image_left_test.cols;
            ...
            FILE* fpt;
            fopen_s(&fpt, "../output/testmatout_left.txt", "w");
            ...
            fprintf_s(fpt, "%d\t%d\t%d :\n",
                ...
                i,
                i);
            fprintf_s(fpt, "flags: %d\tdims: %d\tstep: %lld\t",
                image_left_test.flags,
                image_left_test.dims,
                image_left_test.step.p[0]);
            fprintf_s(fpt, "%lld\n ", image_left_test.step.p[1]);


            int imcol5 = image_left_test.cols;
            for (int j = 0; j < image_left_test.rows; j++)
            {
                for (int k = 0; k < image_left_test.cols; k++)
                {
                    ...
                    fprintf_s(fpt, "%d, ", j);
                    fprintf_s(fpt, "%d ): ", k);
                    fprintf_s(fpt, " data: %d ", image_left_test.at<unsigned char>(j, k));
                    fprintf_s(fpt, " %d ", image_left_test.data[k + (j * imcol5)]);
                    fprintf_s(fpt, "\n");
                }
            }
...
#pragma warning(disable:4996)
            FILE* fpmr1;
            fpmr1 = fopen("../output/matdata_right.txt", "r");
            int vnum11, vnum21;
            int data1;
            int flag1, dim1, row1, col1;
            unsigned long long p11, p21;
            //unsigned char datause;

            fscanf(fpmr1, "%d %d %lld %lld %d %d", &flag1, &dim1, &p11, &p21, &row1, &col1);

            //image_test = image_left;
            
            image_right_test.flags = flag1;
            image_right_test.dims = dim1;
            image_right_test.step.p[0] = p11;
            image_right_test.step.p[1] = p21;
            image_right_test.rows = row1;
            image_right_test.cols = col1;
            static unsigned char ch1[1241 * 376];

            image_right_test.data = ch1;
            int imcol1 = image_right_test.cols;
            for (int j = 0; j < image_right_test.rows; j++)
            {
                for (int k = 0; k < image_right_test.cols; k++)
                {
                    fscanf(fpmr1, "%d %d", &vnum11, &vnum21);
                    fscanf(fpmr1, "%d", &data1);

                    //datause = static_cast<unsigned char>(data);
                    int num1 = k + (j * imcol1);
                    image_right_test.data[num1] = static_cast<unsigned char>(data1);
            ...
            FILE* fpt1;
            fopen_s(&fpt1, "../output/testmatout_right.txt", "w");


            fprintf_s(fpt1, "%d\t%d\t%d :\n",
                i,
                i,
                i);
            fprintf_s(fpt1, "flags: %d\tdims: %d\tstep: %lld\t",
                image_right_test.flags,
                image_right_test.dims,
                image_right_test.step.p[0]);
            fprintf_s(fpt1, "%lld\n ", image_right_test.step.p[1]);


            int imcol3 = image_right_test.cols;
            for (int j = 0; j < image_right_test.rows; j++)
            {
                for (int k = 0; k < image_right_test.cols; k++)
                {
                    ...
                    fprintf_s(fpt1, "%d, ", j);
                    fprintf_s(fpt1, "%d ): ", k);
                    fprintf_s(fpt1, " data: %d ", image_right_test.at<unsigned char>(j, k));
                    fprintf_s(fpt1, " %d ", image_right_test.data[k + (j * imcol3)]);
                    fprintf_s(fpt1, "\n");
                }
            }
...
#pragma warning(disable:4996)
        FILE* fpmr1k;
        fpmr1k = fopen("../output/matdata_k_left.txt", "r");
        int vnum11k, vnum21k;
        float data1k;
        ...
        k_left_test.step.p[0] = p11k;
        k_left_test.step.p[1] = p21k;
        ...
        FILE* fpt1k;
        fopen_s(&fpt1k, "../output/testmatout_k_left_test.txt", "w");


        fprintf_s(fpt1k, "%d\t%d\t%d :\n",
            i,
            i,
            i);
        fprintf_s(fpt1k, "flags: %d\tdims: %d\tstep: %lld\t",
            k_left_test.flags,
            k_left_test.dims,
            k_left_test.step.p[0]);
        fprintf_s(fpt1k, "%lld\n ", k_left_test.step.p[1]);
        ...
        for (int j = 0; j < k_left_test.rows; j++)
        {
            for (int k = 0; k < k_left_test.cols; k++)
            {
                ...
                fprintf_s(fpt1k, "%d, ", j);
                fprintf_s(fpt1k, "%d ): ", k);
                fprintf_s(fpt1k, " data: %f ", k_left_test.at<float>(j, k));
                //fprintf_s(fpt1k, " %d ", k_left_test.data[k + (j * imcol3k)]);
                fprintf_s(fpt1k, "\n");
            }
        }
...
#pragma warning(disable:4996)
        FILE* fpmr1r;
        fpmr1r = fopen("../output/matdata_r_left.txt", "r");
        int vnum11r, vnum21r;
        float data1r;
        ...
        r_left_test.flags = flag1r;
        r_left_test.dims = dim1r;
        r_left_test.step.p[0] = p11r;
        r_left_test.step.p[1] = p21r;
        r_left_test.rows = row1r;
        r_left_test.cols = col1r;
        static unsigned char ch1r[3 * 3];

        r_left_test.data = ch1r;
        //int imcol1k = k_left_test.cols;
        for (int j = 0; j < r_left_test.rows; j++)
        {
            for (int k = 0; k < r_left_test.cols; k++)
            {
                fscanf(fpmr1r, "%d %d", &vnum11r, &vnum21r);
                fscanf(fpmr1r, "%f", &data1r);


                //int num1k = k + (j * imcol1k);
                //k_left_test.data[num1k] = static_cast<unsigned char>(data1k);
                r_left_test.at<float>(j, k) = static_cast<float>(data1r);
        ...
        FILE* fpt1r;
        fopen_s(&fpt1r, "../output/testmatout_r_left_test.txt", "w");


        fprintf_s(fpt1r, "%d\t%d\t%d :\n",
            i,
            i,
            i);
        fprintf_s(fpt1r, "flags: %d\tdims: %d\tstep: %lld\t",
            r_left_test.flags,
            r_left_test.dims,
            r_left_test.step.p[0]);
        fprintf_s(fpt1r, "%lld\n ", r_left_test.step.p[1]);
        ...
        for (int j = 0; j < r_left_test.rows; j++)
        {
            for (int k = 0; k < r_left_test.cols; k++)
            {
                ...
                fprintf_s(fpt1r, "%d, ", j);
                fprintf_s(fpt1r, "%d ): ", k);
                fprintf_s(fpt1r, " data: %f ", r_left_test.at<float>(j, k));
                //fprintf_s(fpt1k, " %d ", k_left_test.data[k + (j * imcol3k)]);
                fprintf_s(fpt1r, "\n");
            }
        }
...
#pragma warning(disable:4996)
        FILE* fpmr1t;
        fpmr1t = fopen("../output/matdata_t_left.txt", "r");
        int vnum11t, vnum21t;
        float data1t;
        ...
        t_left_test.step.p[0] = p11t;
        t_left_test.step.p[1] = p21t;
        t_left_test.rows = row1t;
        t_left_test.cols = col1t;
        static unsigned char ch1t[1 * 4*4];
        ...
        FILE* fpt1t;
        fopen_s(&fpt1t, "../output/testmatout_t_left_test.txt", "w");


        fprintf_s(fpt1t, "%d\t%d\t%d :\n",
            i,
            i,
            i);
        fprintf_s(fpt1t, "flags: %d\tdims: %d\tstep: %lld\t",
            t_left_test.flags,
            t_left_test.dims,
            t_left_test.step.p[0]);
        fprintf_s(fpt1t, "%lld\n ", t_left_test.step.p[1]);
        ...
        for (int j = 0; j < t_left_test.rows; j++)
        {
            for (int k = 0; k < t_left_test.cols; k++)
            {
                ...
                fprintf_s(fpt1t, "%d, ", j);
                fprintf_s(fpt1t, "%d ): ", k);
                fprintf_s(fpt1t, " data: %f ", t_left_test.at<float>(j, k));
                //fprintf_s(fpt1k, " %d ", k_left_test.data[k + (j * imcol3k)]);
                fprintf_s(fpt1t, "\n");
            }
        }
        ...
        //-----------------------------------------------------------------------------------
        /*
        Mat disp_left, disp_left_origin;

        if (i == 1)
        {
            disp_left = computeLeftDisparityMap(image_left_test, image_right_test, SGBM, 0);
            disp_left_origin = computeLeftDisparityMap(image_left, image_right, SGBM, 0);
        }
        ...
        if (i == 1)
        {
            ...
            FILE* fpd;
            fopen_s(&fpd, "../output/test_left.txt", "w");
            ...
            FILE* fpd1;
            fopen_s(&fpd1, "../output/test_left_origin.txt", "w");

            fprintf_s(fpd1, "%d\t%d\t%lld\t%lld\t",
                disp_left_origin.flags,
                disp_left_origin.dims,
                disp_left_origin.step.p[0],
                disp_left_origin.step.p[1]
            );
            fprintf_s(fpd1, "%d\t%d\n", disp_left_origin.rows, disp_left_origin.cols);

            for (int j = 0; j < disp_left_origin.rows; j++)
            {
                for (int k = 0; k < disp_left_origin.cols; k++)
                {
                    fprintf_s(fpd1, " %d ", j);
                    fprintf_s(fpd1, " %d ", k);
                    fprintf_s(fpd1, " %f ", disp_left_origin.at<float>(j, k));
                    //cout << "type: " << disp_left_origin.type() << endl;//5
                    fprintf_s(fpd1, "\n");
                }
            }
        ...
        if (i == 1)
        {
            depth = stereo_2_depth_1(image_left_test, image_right_test, handler.P0, handler.P1, SGBM, false, true);
            depth_origin = stereo_2_depth(image_left, image_right, handler.P0, handler.P1, SGBM, false, true);
        }
        //---------------------------------------------------------------------------------------------------------
        /*
        if (i == 1)
