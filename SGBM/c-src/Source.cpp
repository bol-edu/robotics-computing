...
int main()
{
	...
	***************************************************************************************/
	...
	/*
	Mat depth_map123 = Mat::ones(376, 1241, CV_32F);

	FILE* fpd2123;
	fopen_s(&fpd2123, "../output/test_depth_map123.txt", "w");

	fprintf_s(fpd2123, "%d\t%d\t%lld\t%lld\t",
		depth_map123.flags,
		depth_map123.dims,
		depth_map123.step.p[0],
		depth_map123.step.p[1]
	);
	fprintf_s(fpd2123, "%d\t%d\n", depth_map123.rows, depth_map123.cols);


	for (int j = 0; j < depth_map123.rows; j++)
	{
		for (int k = 0; k < depth_map123.cols; k++)
		{
			fprintf_s(fpd2123, " %d ", j);
			fprintf_s(fpd2123, " %d ", k);
			fprintf_s(fpd2123, " %f ", depth_map123.at<float>(j, k));
