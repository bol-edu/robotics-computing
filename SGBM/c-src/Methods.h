...
{
    FILE* fpt_level;
    fopen_s(&fpt_level, "../output/disp_new.txt", "w");
...
Mat computeLeftDisparityMap(Mat img_left, Mat img_right, int matcher_name, bool rgb)
{
    ...
    if (matcher_name == SGBM)
    {
        ...
        //disp_left.convertTo(disp_left, CV_32F, 1.0 / 16);//short->double
...
Mat stereo_2_depth(Mat img_left, Mat img_right, Mat k_left, Mat t_left, Mat t_right, bool matcher, bool rgb, bool rectified)

{
    //Methods method;
    // Compute disparity map
    Mat disp = computeLeftDisparityMap(img_left, img_right, SGBM, 0);
